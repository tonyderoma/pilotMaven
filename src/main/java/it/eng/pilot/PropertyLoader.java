...
/**
 * Classe per il caricamento dei file di properties.
 * 
...
class PropertyLoader {
	...
	private static Properties loadPropertiesContent(String name, Class c) {
		if (name == null)
			throw new IllegalArgumentException("null input: name");
		...
		try {
			if (name.indexOf(".") <= 1) {
				name = c.getPackage().getName() + "." + name;
			}
			name = name.replace('.', '/');
			if (!name.endsWith(SUFFIX))
				name = name.concat(SUFFIX);
			...
			// Returns null on lookup failures:
			in = c.getClassLoader().getResourceAsStream(name);
			if (in != null) {
				result = new Properties();
				result.load(in); // Can throw IOException
			}
	...
	public static Properties loadProperties(final String name, Class c) {
		return loadPropertiesContent(name, c);
	}
